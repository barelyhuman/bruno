import { wsConnect, wsDisconnect, wsSendMessage, wsClear, wsConnectSuccess, wsConnectError, wsMessageReceived, wsMessageSent } from './index.js';
...
export const connectWebSocket = ({ itemUid, url }) => (dispatch) => {
  ...
  return callIpc('renderer:ws-connect', { itemUid, url, options: { headers } })
    .then(() => dispatch(wsConnectSuccess({ itemUid })))
    .catch((error) => dispatch(wsConnectError({ itemUid, error: error.message })));
};
